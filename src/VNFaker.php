...
class VNFaker
{
    ...
    const FILE_ADDRESS      = 'vn_structure.json';
    ...
    */
    ...
    {
        return strtolower(self::str_clean(self::vnToString($fullname ?? self::fullname()), true));
    }

    public static function email($prefix = null, array $domain = [])
    {
        $email = '@';
        if (!$domain) {
        ...
        return strtolower(self::username($prefix) . $email);
    }
    ...
    {
        $json     = file_get_contents(__DIR__ . self::FILE_PATH_FOLDER . "/" . self::FOLDER_JSON . "/" . self::FILE_ADDRESS);
        $addr     = json_decode($json, JSON_UNESCAPED_UNICODE);
